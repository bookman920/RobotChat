...
namespace WeChat.NET.Controls
{
    ...
    public partial class WChatBox : UserControl
    {
            ...
            set
            {
                if (value != _friendUser)
                {
                    ...
                    if (_friendUser != null)
                    {
                        ...
                        var dic = _friendUser.RecvedMsg.Concat(_friendUser.SentMsg);
                        dic = dic.OrderBy(i => i.Time);
                        foreach (var p in dic)  //恢复聊天记录
                        {
                            if (p.From == _friendUser.Id)
                            {
                                ShowReceiveMsg(p);
                            }
                            else
        ...
        //自己
        private BaseContact _meUser;
        public BaseContact MeUser
        {
            get
            {
                return _meUser;
